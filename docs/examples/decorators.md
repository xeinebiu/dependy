...
Wrap a resolved instance with composable transformations.
...
- Decorators are applied in list order after the factory.
- Singletons are decorated once and cached. Transients are decorated on every call.
- Decorators can resolve any type from the module via the `resolve` parameter.
